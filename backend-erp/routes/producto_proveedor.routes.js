// Rutas HTTP (GET/POST/PUT/DELETE) para producto_proveedor.
...
} = require('../app/services/producto_proveedor.service');

const router = express.Router();

router.get('/', async (req, res) => {
  try {
    res.json(await obtenerProductoProveedors());
  } catch (error) {
    console.error('Error al consultar producto_proveedor:', error);
    res.status(500).json({ mensaje: 'No se pudieron obtener los registros de producto_proveedor.' });
  }
});

router.get('/:id', async (req, res) => {
  try {
    const registro = await obtenerProductoProveedorPorId(req.params.id);
...
router.post('/', async (req, res) => {
  try {
    const nuevoRegistro = await crearProductoProveedor(req.body);
...
router.put('/:id', async (req, res) => {
  try {
    const registroActualizado = await actualizarProductoProveedor(req.params.id, req.body);
...
router.delete('/:id', async (req, res) => {
  try {
    await eliminarProductoProveedor(req.params.id);
    res.status(204).send();
  } catch (error) {

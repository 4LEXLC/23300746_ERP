...
const {
  ...
  actualizarPuesto,
  eliminarPuesto,
} = require('../app/services/puesto.service');

const router = express.Router();

router.get('/', async (req, res) => {
  try {
    res.json(await obtenerPuestos());
  } catch (error) {
    console.error('Error al consultar puesto:', error);
    res.status(500).json({ mensaje: 'No se pudieron obtener los registros de puesto.' });
  }
});

router.get('/:id', async (req, res) => {
  try {
    const registro = await obtenerPuestoPorId(req.params.id);
...
router.post('/', async (req, res) => {
  try {
    const nuevoRegistro = await crearPuesto(req.body);
...
router.put('/:id', async (req, res) => {
  try {
    const registroActualizado = await actualizarPuesto(req.params.id, req.body);
...
router.delete('/:id', async (req, res) => {
  try {
    await eliminarPuesto(req.params.id);
    res.status(204).send();
  } catch (error) {

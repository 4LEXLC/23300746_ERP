...
const {
  ...
  actualizarTicket,
  eliminarTicket,
} = require('../app/services/ticket.service');

const router = express.Router();

router.get('/', async (req, res) => {
  try {
    res.json(await obtenerTickets());
  } catch (error) {
    console.error('Error al consultar ticket:', error);
    res.status(500).json({ mensaje: 'No se pudieron obtener los registros de ticket.' });
  }
});

router.get('/:id', async (req, res) => {
  try {
    const registro = await obtenerTicketPorId(req.params.id);
...
router.post('/', async (req, res) => {
  try {
    const nuevoRegistro = await crearTicket(req.body);
...
router.put('/:id', async (req, res) => {
  try {
    const registroActualizado = await actualizarTicket(req.params.id, req.body);

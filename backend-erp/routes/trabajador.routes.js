...
const {
  ...
  eliminarTrabajador,
} = require('../app/services/trabajador.service');

const router = express.Router();

router.get('/', async (req, res) => {
  try {
    res.json(await obtenerTrabajadors());
  } catch (error) {
    console.error('Error al consultar trabajador:', error);
    res.status(500).json({ mensaje: 'No se pudieron obtener los registros de trabajador.' });
  }
});

router.get('/:id', async (req, res) => {
  try {
    const registro = await obtenerTrabajadorPorId(req.params.id);
...
router.post('/', async (req, res) => {
  try {
    const nuevoRegistro = await crearTrabajador(req.body);
...
router.put('/:id', async (req, res) => {
  try {
    const registroActualizado = await actualizarTrabajador(req.params.id, req.body);
...
router.delete('/:id', async (req, res) => {
  try {
    await eliminarTrabajador(req.params.id);
    res.status(204).send();
  } catch (error) {

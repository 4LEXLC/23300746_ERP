...
const {
  ...
  actualizarProveedor,
  eliminarProveedor,
} = require('../app/services/proveedor.service');

const router = express.Router();

router.get('/', async (req, res) => {
  try {
    res.json(await obtenerProveedors());
  } catch (error) {
    console.error('Error al consultar proveedor:', error);
    res.status(500).json({ mensaje: 'No se pudieron obtener los registros de proveedor.' });
  }
});

router.get('/:id', async (req, res) => {
  try {
    const registro = await obtenerProveedorPorId(req.params.id);
...
router.post('/', async (req, res) => {
  try {
    const nuevoRegistro = await crearProveedor(req.body);
...
router.put('/:id', async (req, res) => {
  try {
    const registroActualizado = await actualizarProveedor(req.params.id, req.body);
